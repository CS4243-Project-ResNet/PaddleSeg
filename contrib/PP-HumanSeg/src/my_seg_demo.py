...
def my_seg_image(args):
    ...
    img = cv2.imread(args.img_path)
    bg_img = get_bg_img(args.bg_img_path, img.shape)
    out_img, alpha = predictor.my_run(img, bg_img)
    alpha_dir = args.save_dir[:-4] + "alpha" + args.save_dir[-4:]
    alpha_save = np.uint8(alpha*255)
    # for i in range(alpha_save.shape[0]):
    #     for j in range(alpha_save.shape[1]):
    #         if alpha_save[i, j] != 0 :
    #             print('{}, {}: {}'.format(i, j, alpha_save[i, j]))
    # print(alpha_dir)
    # print(alpha_save)
    # cv2.imwrite(args.save_dir, out_img)
    cv2.imwrite(args.save_dir, alpha_save)
